...
export const getAuthorDescription = [
  ...
  "It does not list the person's works. The catalogue links them, and no tool here walks that link: search_works reads titles, so a work is found by its title rather than by its author.",
...
export const getAuthorOutput = z.object({
  author: z.object({
    ...
    catalogue_url: z.string().nullable().describe("The record in the BnF general catalogue."),
...
export async function runGetAuthor(client: BnfClient, args: GetAuthorArgs): Promise<ToolResult> {
  try {
    ...
    if (Object.keys(data.sameAs).length > 0) {
      notes.push(
        "The addresses under 'same_as' are the BnF's own alignments to other files. Following one is how to reach a biography, which this catalogue does not hold.",
      );
    }
